...
namespace PenaltyKing.Tests
{
    public sealed class ActionRevisionTests
    {
        ...
        [UnityTest] public IEnumerator WindupContactsBallThenGoalSettlesWithFrontFansStill()
        {
            ...
            p.Sample(goal,.3f);Assert.That(p.UsesBoneRigs,Is.True);Assert.That(p.BallPosition,Is.EqualTo(p.BallRestPosition));
            p.Sample(goal,ShotPresentation.ContactTime);Assert.That(player.enabled,Is.False,"Bone artwork replaces the pose image");
            ...
            p.ResetPose();Assert.That(p.UsesBoneRigs,Is.True);Assert.That(p.GoalCelebrating,Is.False);
            yield return null;
        }
    }
}
