...
namespace PenaltyKing.Tests
{
    public sealed class ActionRevisionTests
    {
        ...
        [UnityTest] public IEnumerator WindupContactsBallThenGoalSettlesWithFrontFansStill()
        {
            ...
            p.Sample(goal,.3f);Assert.That(player.sprite.name,Is.EqualTo("StrikerShot12_03"));Assert.That(p.BallPosition,Is.EqualTo(p.BallRestPosition));
            p.Sample(goal,ShotPresentation.ContactTime);Assert.That(player.sprite.name,Is.EqualTo("StrikerShot12_08"));
            ...
            Assert.That(player.sprite.name,Is.EqualTo("Adult_10"));
            ...
            p.ResetPose();Assert.That(player.sprite.name,Is.EqualTo("StrikerShot12_00"));Assert.That(p.GoalCelebrating,Is.False);
            yield return null;
        }
    }
}
